//! Lucifer Tauri application library.
//!
//! Tauri mobile (iOS/Android) drives the app through a **library** entry point
//! (`run()` below, tagged `mobile_entry_point`), while desktop launches via the
//! thin `src/main.rs` binary calling the same `run()`. Desktop-only chrome —
//! the tray icon and the global overlay hotkey — lives in the `cfg(desktop)`
//! [`hotkey`] module; everything else (state handles, IPC commands, settings,
//! local stores, inference selection) is platform-portable.

use lucifer_desktop_bindings::{
    handlers, ClientHandle, EdgeStoreHandle, InferenceHandle, OfflineQueueHandle, SettingsHandle,
};
use tauri::Manager;

/// Shared Tauri entry point for desktop (via `main.rs`) and mobile (via the
/// generated platform projects).
#[cfg_attr(mobile, tauri::mobile_entry_point)]
pub fn run() {
    init_tracing();
    tracing::info!("Lucifer starting");

    let builder = tauri::Builder::default();

    #[cfg(desktop)]
    let builder = builder.plugin(hotkey::global_shortcut_plugin());
    ...
    builder
        ...
        .setup(|app| {
            ...
            #[cfg(desktop)]
            {
                hotkey::register_overlay_shortcut(app.handle())?;
                hotkey::install_tray(app.handle())?;
            }
            open_default_local_stores(app.handle());
            tracing::info!("Lucifer ready");
            Ok(())
        })
        .run(tauri::generate_context!())
        .expect("failed to launch Lucifer");
}

/// Load persisted settings and seed the async `SettingsHandle` so the IPC
/// commands serve the same data. On desktop this also resolves the overlay
/// hotkey binding from the stored accelerator.
fn load_settings(app: &tauri::AppHandle) {
    let path = match app.path().app_data_dir() {
        ...
        Err(e) => {
            ...
            #[cfg(desktop)]
            hotkey::set_default_binding();
            return;
        }
    };

    #[cfg(desktop)]
    hotkey::resolve_overlay_binding(&path);
...
/// Desktop-only chrome: tray icon + global overlay hotkey. Mobile builds omit
/// this entire module (no tray/global-shortcut paradigm there).
#[cfg(desktop)]
mod hotkey {
    use std::sync::OnceLock;

    use lucifer_desktop_bindings::settings::AppSettings;
    use tauri::{
        menu::{Menu, MenuItem},
    ...
    /// The (modifiers, key) the overlay toggle is bound to, resolved from
    /// settings at startup. Changing the hotkey applies on next launch.
    static OVERLAY_BINDING: OnceLock<(Modifiers, Code)> = OnceLock::new();

    pub fn set_default_binding() {
        let _ = OVERLAY_BINDING.set((Modifiers::SUPER, Code::Space));
    }

    /// Read the stored accelerator from `settings.json` and pin the binding.
    pub fn resolve_overlay_binding(path: &std::path::Path) {
    ...
    /// The global-shortcut plugin wired to toggle the overlay window.
    ...
    pub fn register_overlay_shortcut(app: &tauri::AppHandle) -> tauri::Result<()> {
        let (mods, code) = OVERLAY_BINDING
            .get()
            .copied()
            .unwrap_or((Modifiers::SUPER, Code::Space));
