...
Stages (in order):
  ...
  3. Context profiling      → score + rank chunks by relevance (BM25)
...
from master.token_optimizer.context_profiler import ContextProfiler
...
class TokenOptimizer:
    ...
    def __init__(
        ...
        self._profiler = ContextProfiler()
        self._deduplicator = SemanticDeduplicator(hamming_threshold=dedup_threshold)
    ...
    async def prepare(
        ...
        scored_chunks = self._profiler.profile(prompt, chunks)
        stages.append("relevance_score")

        # ── Stage 4: Deduplication (SimHash) ─────────────────────────────────
        deduped = self._deduplicator.deduplicate(scored_chunks)
        if len(scored_chunks) != len(deduped):
            log.debug("token_optimizer.dedup", dropped=len(scored_chunks) - len(deduped))
        stages.append("dedup")
