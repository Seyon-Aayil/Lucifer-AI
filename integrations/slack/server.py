...
async def mcp_endpoint(request: Request) -> JSONResponse:
    ...
    if method == "tools/list":
        tools = [
            ...
            {"name": "send_message", "description": "Send a message to a Slack channel or DM",
             "requiresConfirmation": True,
             "inputSchema": {"type": "object", "required": ["channel", "text"],
                             "properties": {"channel": {"type": "string"},
                                            "text": {"type": "string"}}}},
            {"name": "search_messages", "description": "Search Slack messages by query",
...
async def _dispatch(tool_name: str, args: dict[str, Any]) -> dict[str, Any]:
    ...
    if tool_name == "read_channel":
        async with httpx.AsyncClient() as c:
            r = await c.get(f"{_SLACK_API}/conversations.history", headers=_headers(),
    ...
    if tool_name == "send_message":
        async with httpx.AsyncClient() as c:
            r = await c.post(f"{_SLACK_API}/chat.postMessage", headers=_headers(),
    ...
    if tool_name == "search_messages":
        async with httpx.AsyncClient() as c:
            r = await c.get(f"{_SLACK_API}/search.messages", headers=_headers(),
                            params={"query": args["query"], "count": int(args.get("limit", 20))})
            r.raise_for_status()
            data = _check_ok(r.json())
        matches = data.get("messages", {}).get("matches", [])
        results = [{"user": m.get("username", ""), "text": m.get("text", ""),
